...
ISO_OUT  := output/disc
ISO_COMPRESS := output/iso
WBFS_COMPRESS := output/wbfs
...
# -ldi / -liso9660: mounts the optical disc as a POSIX-readable filesystem
# under the "dvd:/" prefix (libogc's libdi + libiso9660 devoptab). Without
# these, disc builds (ISO/WBFS) have no way to read anything from the disc
# at runtime -- fatInit() below only ever mounts sd:/ and usb:/.
LIBS := -ldi -liso9660 -lgrrlib -lpngu $(FREETYPE_LIBS) -lbrotlidec -lbrotlicommon -ljpeg -lpng -lz -lfat -lwiiuse -lbte -lmad -lasnd -logc -lm
...
iso: check-wit $(TARGET).dol
	@echo "==> Packing ISO"
	@chmod +x $(ROOT_DIR)/$(ISO_SCRIPT_SH) 2>/dev/null || true
	@TARGET=$(TARGET) WIT_PATH="$(WIT_PATH)" "$(ROOT_DIR)/$(ISO_SCRIPT_SH)" "$(ROOT_DIR)/$(TARGET).dol" iso

wbfs: check-wit $(TARGET).dol
	@echo "==> Packing WBFS"
	@chmod +x $(ROOT_DIR)/$(ISO_SCRIPT_SH) 2>/dev/null || true
	@TARGET=$(TARGET) WIT_PATH="$(WIT_PATH)" "$(ROOT_DIR)/$(ISO_SCRIPT_SH)" "$(ROOT_DIR)/$(TARGET).dol" wbfs
...
ifneq (,$(wildcard assets))
	...
	@cd $(ROOT_DIR)/$(HB_OUT)/.. && zip -r $(TARGET).zip $(notdir $(HB_OUT))/ >/dev/null && echo "Homebrew zip -> $(HB_OUT)/../$(TARGET).zip"
...
run: all
	@echo "Homebrew: $(HB_OUT)/../$(TARGET).zip — copy to /apps/ on SD card"
	@echo "ISO:      $(ISO_COMPRESS)/$(TARGET).zip — use in Dolphin or burn"
	@echo "WBFS:     $(WBFS_COMPRESS)/$(TARGET).zip — use with USB Loader"

help:
	@echo "Targets:"
	@echo "  make            Build $(TARGET).dol + all three zips (homebrew/iso/wbfs)"
	@echo "  make homebrew   Build $(TARGET).dol + output/homebrew/$(TARGET).zip only"
	@echo "  make iso        Build $(TARGET).dol + output/iso/$(TARGET).zip only"
	@echo "  make wbfs       Build $(TARGET).dol + output/wbfs/$(TARGET).zip only"
...
clean-iso:
	...
	@rm -fr $(ROOT_DIR)/$(HB_OUT)
	@rm -fr $(ROOT_DIR)/$(ISO_OUT)/$(TARGET).iso $(ROOT_DIR)/$(ISO_OUT)/$(TARGET).wbfs
	@rm -fr $(ROOT_DIR)/$(ISO_COMPRESS)/$(TARGET).zip $(ROOT_DIR)/$(WBFS_COMPRESS)/$(TARGET).zip
	@rm -fr $(ROOT_DIR)/$(HB_OUT)/../$(TARGET).zip
