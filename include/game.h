...
typedef struct GameContext {
    ...
    /* Right-side level progress bar (see Level_GetLevelProgress()). Eased
     * toward its target each frame in Game_Update rather than snapping to
     * it, so it visibly slows and settles at a wave's tick mark instead of
     * jumping straight to it. Reset to 0 at the start of every level. */
    f32       levelProgressDisplayed;
