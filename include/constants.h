...
/* How close (in tile-space x) a projectile must get to a zombie before it
 * "connects". Without this, AdvanceProjectiles's collision search picked
 * the nearest zombie *anywhere ahead in the row*, so a pea fired at x=0.6
 * could instantly detonate on a zombie sitting at x=9 a frame later --
 * dealing damage correctly but never visibly traveling, which reads as the
 * projectile simply not existing. */
#define PROJECTILE_HIT_DISTANCE_TILES 0.35f
...
/* "A huge wave of zombies is approaching!" banner (Adventure Mode only),
 * shown right as the level's last/biggest wave begins so the player gets a
 * heads up. The red text stays on screen for HUGE_WAVE_MESSAGE_FRAMES, then
 * there's an extra HUGE_WAVE_POST_DELAY_FRAMES of empty lawn before the
 * wave's zombies start spawning -- 3 seconds total from the moment the
 * message appears, no button press required. */
#define HUGE_WAVE_MESSAGE_FRAMES    (TARGET_FPS * 2)  /* message on screen 2s   */
#define HUGE_WAVE_POST_DELAY_FRAMES (TARGET_FPS * 1)  /* +1s after it vanishes */

/* Level progress bar -- right-hand side of the HUD, under the world/level
 * label. Shows overall progress through the current level's wave set, with
 * a thin tick mark at each wave boundary. Its displayed value is eased
 * toward the real target (see Level_GetLevelProgress()) rather than
 * snapping to it, which is what makes it visibly slow down and settle at
 * a tick while the game is waiting on the next zombie spawn (or, for the
 * level's last wave, during the huge-wave countdown) before picking back
 * up once spawning resumes. */
#define LEVEL_PROGRESS_BAR_WIDTH    150
#define LEVEL_PROGRESS_BAR_HEIGHT   10
#define LEVEL_PROGRESS_BAR_MARGIN_X 20
#define LEVEL_PROGRESS_BAR_MARGIN_Y 42   /* below the "World - Level N" label */
#define LEVEL_PROGRESS_EASE_FACTOR  0.04f
