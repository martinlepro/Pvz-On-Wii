                ...
                case PLANT_CHOMPER: {
                    ...
                    if (zi >= 0 && cell->shootTimer == 0) {
                        ...
                        if (target->health <= 400) {
                            ...
                            cell->shootTimer = 42 * TARGET_FPS; /* ~42s chewing */
                        } else {
                            target->health -= 40;
...
static void AdvanceProjectiles(GameContext* game) {
    ...
    for (int i = 0; i < MAX_PROJECTILES; i++) {
        ...
        for (int zi = 0; zi < MAX_ZOMBIES; zi++) {
            ...
            if (z->x < p->x) continue;
            /* Must have actually traveled to the zombie, not just be anywhere
             * earlier in the same row -- see PROJECTILE_HIT_DISTANCE_TILES. */
            if (z->x - p->x > PROJECTILE_HIT_DISTANCE_TILES) continue;
...
void Combat_Update(GameContext* game) {
    ...
    /* 3) Decrement spikeweed timers (they are not handled in the shooter loop) */
    for (s8 row = 0; row < (s8)game->rowCount; row++) {
        for (s8 col = 0; col < GRID_COLS; col++) {
            GridCell* cell = &game->grid[row][col];
            if ((cell->plant == PLANT_SPIKEWEED || cell->plant == PLANT_SPIKEROCK) && cell->shootTimer > 0)
                cell->shootTimer--;
        }
    }
    /* 4) Spikeweed/Spikerock */
    CheckSpikeweed(game);
