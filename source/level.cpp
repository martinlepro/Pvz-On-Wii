...
f32 Level_GetLevelProgress(const GameContext* game) {
    if (game->state == GAME_STATE_LEVEL_WON) return 1.0f;
    if (game->state != GAME_STATE_PLAYING) return -1.0f;
    if (game->waveCount == 0) return -1.0f;

    u32 totalZombies = 0;
    for (u8 w = 0; w < game->waveCount; w++)
        totalZombies += game->waves[w].zombieCount;
    if (totalZombies == 0) return -1.0f; /* guard against div-by-zero */

    /* Every previous wave counts as fully "done"; the current wave counts
     * however many of its zombies have been spawned so far. This holds
     * steady (rather than creeping forward) whenever spawnedCount isn't
     * changing -- i.e. while waiting on the next spawn timer, or during the
     * huge-wave countdown before the level's last wave -- which is exactly
     * what makes the eased display in render.cpp settle at a tick mark
     * instead of drifting past it. */
    u32 doneZombies = 0;
    for (u8 w = 0; w < game->currentWave; w++)
        doneZombies += game->waves[w].zombieCount;
    doneZombies += game->waves[game->currentWave].spawnedCount;

    f32 progress = (f32)doneZombies / (f32)totalZombies;
    if (progress > 1.0f) progress = 1.0f;
    if (progress < 0.0f) progress = 0.0f;
    return progress;
}

u8 Level_GetWaveTickFractions(const GameContext* game, f32* outFractions) {
    if (game->waveCount == 0) return 0;

    u32 totalZombies = 0;
    for (u8 w = 0; w < game->waveCount; w++)
        totalZombies += game->waves[w].zombieCount;
    if (totalZombies == 0) return 0;

    u8 count = 0;
    u32 cumulative = 0;
    /* One tick per wave boundary; skip the very last one since it's just
     * the right edge of the bar. */
    for (u8 w = 0; w + 1 < game->waveCount; w++) {
        cumulative += game->waves[w].zombieCount;
        outFractions[count++] = (f32)cumulative / (f32)totalZombies;
    }
    return count;
}
