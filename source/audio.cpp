...
void Audio_PlayMenuBGM(void)
{
    /* Prefer a dedicated title/menu track if one is dropped in later, same
     * "no code changes needed" convention as the rest of assets/. Falls back
     * to the World 1 theme so the title screen isn't silent in the
     * meantime. */
    FILE* probe = fopen("assets/music/menu.wav", "rb");
    if (probe)
    {
        fclose(probe);
        Audio_PlayBGM("assets/music/menu.wav");
        return;
    }
    Audio_PlayBGM("assets/music/world 1.wav");
}
