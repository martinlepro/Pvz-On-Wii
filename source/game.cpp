...
static bool TryPlantAtCell(GameContext* game, s8 col, s8 row, PlantType type)
{
    ...
    /* Lily Pad special placement — platform, not a plant */
    ...
    /* Flower Pot special placement — platform, not a plant */
    ...
    /* Water rows: must have a lily pad to plant non-aquatic plants */
    if (isWater)
    {
        /* Aquatic plants can be placed directly in water */
        if (type == PLANT_TANGLE_KELP || type == PLANT_SEA_SHROOM)
        {
            /* Can place directly on water, no lily pad needed */
        }
        else
        {
            /* Non-aquatic plants need a lily pad */
            if (!cell->hasLilyPad)
                return false;
        }
    }

    /* Roof world: must have a flower pot to plant non-pot plants */
...
static bool SpawnSunDropAt(GameContext* game, f32 x, f32 targetY, bool fromSunflower)
{
    ...
    /* Pool full: if from sunflower, replace the oldest sunflower sun */
    if (fromSunflower)
    {
        s8 oldest = -1;
        u32 maxAge = 0;
        for (u8 i = 0; i < MAX_SUN_DROPS; ++i)
        {
            if (game->suns[i].fromSunflower && game->suns[i].restFrames > maxAge)
            {
                oldest = (s8)i;
                maxAge = game->suns[i].restFrames;
            }
        }
        if (oldest >= 0)
        {
            SunDrop* s = &game->suns[oldest];
            s->x         = x;
            s->y         = targetY - 20.0f;
            s->targetY   = targetY;
            s->peakY     = targetY - 35.0f;
...
void Game_Init(GameContext* game)
{
    ...
    game->hugeWaveMessageTimer = 0;
    game->hugeWaveDelayTimer = 0;
    game->levelProgressDisplayed = 0.0f;
}
...
void Game_Reset(GameContext* game)
{
    ...
    /* Vertically center the lawn when this level uses fewer than the full
     * GRID_ROWS rows (e.g. the very first level, which is a single row) --
     * cellH is always sized for GRID_ROWS, so any unused rows just become
     * blank margin split evenly above and below the active grid instead of
     * leaving it pinned to the top. */
    {
        u8 visibleRows = (game->rowCount > 0 && game->rowCount <= GRID_ROWS) ? game->rowCount : GRID_ROWS;
        game->lawnY = LAWN_MARGIN_Y + (s16)(((GRID_ROWS - visibleRows) * game->cellH) / 2);
    }
...
void Game_ConfirmSelection(GameContext* game)
{
    ...
    if (game->isSurvivalReselect)
    {
        ...
        /* Endless cycles through freshly-generated 3-wave batches; Easy/Hard
         * already have their whole 5/10-wave plan built up front (see
         * Survival_BuildWavePlan), so there's nothing to regenerate -- just
         * resume playing at the wave that was already advanced to. */
...
void Game_HandleInput(GameContext* game, const InputState* input)
{
    ...
    if (input->hoveredSeedSlot >= 0 && input->hoveredSeedSlot < game->selectedCount)
...
void Game_Update(GameContext* game, const InputState* input)
{
    ...
    /* Ease the right-side level progress bar toward its real target instead
     * of snapping to it. The target itself holds steady while spawnedCount
     * isn't moving (waiting on the next spawn timer, or during the huge-wave
     * countdown before the level's last wave), so this easing is what makes
     * the bar visibly slow down and settle right at a wave's tick mark
     * rather than jumping straight to it, then pick back up once the next
     * zombie actually spawns. */
    {
        f32 targetProgress = Level_GetLevelProgress(game);
        if (targetProgress >= 0.0f)
        {
            f32 diff = targetProgress - game->levelProgressDisplayed;
            game->levelProgressDisplayed += diff * LEVEL_PROGRESS_EASE_FACTOR;
            if (fabsf(diff) < 0.0015f)
                game->levelProgressDisplayed = targetProgress;
        }
    }
