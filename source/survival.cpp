...
void Survival_OnFlagCleared(GameContext* game)
{
    ...
    if (diff == SURVIVAL_DIFF_ENDLESS && game->wavesSinceReselect >= 2)
    {
        game->wavesSinceReselect = 0;
    ...
    {
        u8 totalWaves = (diff == SURVIVAL_DIFF_EASY) ? 5 : 10;
        u8 wavesPerFlag = 1;
        u8 flagsPerReselect = 2;
        u8 wavesBeforeReselect = totalWaves;
        if (game->survivalFlags * wavesPerFlag < wavesBeforeReselect)
        {
            if (game->wavesSinceReselect >= flagsPerReselect)
            {
                game->wavesSinceReselect = 0;
                game->isSurvivalReselect = true;
            }
